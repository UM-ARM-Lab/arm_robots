...
DualGripperShim::DualGripperShim(std::string const robot_namespace, ros::NodeHandle const &nh,
  ...
  auto const default_traj_name =
      ros::names::append(robot_namespace, "both_arms_trajectory_controller/follow_joint_trajectory");
  auto const traj_name = ROSHelpers::GetParam<std::string>(ph, "traj_name", default_traj_name);
