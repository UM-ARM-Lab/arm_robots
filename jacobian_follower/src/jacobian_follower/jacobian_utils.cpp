...
Pose lookupTransform(tf2_ros::Buffer const &buffer, std::string const &parent_frame, std::string const &child_frame,
  ...
  if (!buffer.canTransform(parent_frame, child_frame, target_time)) {
    ROS_WARN_STREAM_NAMED(LOGGER_NAME, "Unable to lookup transform between " << parent_frame << " and " << child_frame
...
Matrix6Xd getJacobianServoFrame(moveit::core::JointModelGroup const *const jmg, robot_state::RobotState const &state,
