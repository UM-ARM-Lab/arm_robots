...
PlanResult JacobianFollower::plan(std::string const &group_name,
                                  std::vector<std::string> const &tool_names,
                                  std::vector<Eigen::Vector4d> const &preferred_tool_orientations,
                                  std::vector<std::vector<Eigen::Vector3d>> const &grippers,
                                  double const max_velocity_scaling_factor,
...
PlanResult JacobianFollower::plan(planning_scene_monitor::LockedPlanningSceneRW &planning_scene,
    ...
    for (size_t waypoint_idx = 0; waypoint_idx < n_points; ++waypoint_idx) {
        ...
        auto const[traj, gripper_reached_target] = moveInWorldFrame(planning_scene,
                                                                    group_name,
                                                                    tool_names,
                                                                    preferred_tool_orientations_q,
                                                                    start_state,
    ...
    if (!time_param_.computeTimeStamps(robot_trajectory, max_velocity_scaling_factor,
                                       max_acceleration_scaling_factor)) {
...
JacobianFollower::compute_IK_solutions(geometry_msgs::Pose target_pose, const std::string &group_name) {
...
geometry_msgs::Pose JacobianFollower::computeFK(const std::vector<double>& joint_angles, const std::string& group_name){
    ...
    // visualize interpolated path
    {
        ...
        for (auto tool_idx = 0ul; tool_idx < num_ees; ++tool_idx) {
            ...
            for (std::size_t step_idx = 0; step_idx < steps; ++step_idx) {
                ...
                auto const ratio =
                        static_cast<float>(step_idx) / static_cast<float>(std::max<std::size_t>(steps - 1, 1));
    ...
    for (int itr = 0; itr < maxItr; itr++) {
        ...
        do {
            ...
            const Eigen::VectorXd positionCorrectionStep =
                    positionMagnitude *
                    EigenHelpers::UnderdeterminedSolver(partialPositionJacobian, posErrorVec, dampingThreshold,
                                                        damping);
            ...
            const auto rotationMagnitude = (effectiveRotationError > stepSize) ? stepSize / effectiveRotationError
                                                                               : 1.0;
            const Eigen::VectorXd rotationCorrectionStep =
                    rotationMagnitude *
                    EigenHelpers::UnderdeterminedSolver(partialRotationJacobian, drotEffective, dampingThreshold,
                                                        damping);
            ...
            currConfig += step;
            newBadJoints =
                    ((currConfig.array() < lowerLimit.array()) || (currConfig.array() > upperLimit.array())) &&
                    goodJoints;
