...
def main():
    ...
    val.plan_to_position_cartesian(val.right_arm_group, val.right_tool_name, [0.5, 0.4, 0.4], step_size=0.01)

    myinput("Follow jacobian to pose 4?")
    val.store_current_tool_orientations([val.right_tool_name])
    val.follow_jacobian_to_position('both_arms', [val.right_tool_name], [[[0.3, 0.3, 0.3]]], vel_scaling=1.0)
